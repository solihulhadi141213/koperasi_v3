<?php
    ...
    if (empty($SessionIdAkses)) {
        ...
        if (empty($_GET['lembaga']) || empty($_GET['tahun']) || empty($_GET['bulan'])) {
            die('Nama Lembaga, Tahun, dan Bulan tidak boleh kosong!');
        } else {
            $lembaga = validateAndSanitizeInput($_GET['lembaga']);
            $tahun = validateAndSanitizeInput($_GET['tahun']);
            $bulan = validateAndSanitizeInput($_GET['bulan']);
            $keyword = "$tahun-$bulan";
            ...
            $sheet->setCellValue('A3', 'Periode : ' . getNamaBulan($bulan) . ' ' . $tahun);

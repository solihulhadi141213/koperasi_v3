<?php
    ...
    if (empty($SessionIdAkses)) {
        ...
        //Proses Hapus
        $Hapus = mysqli_query($Conn, "DELETE FROM transaksi_bulk WHERE kategori='$kategori_transaksi' AND id_akses='$SessionIdAkses'") or die(mysqli_error($Conn));
        if ($Hapus) {
            //Apabila Berhasil Simpan Log
            $kategori_log="Transaksi Penjualan";
            $deskripsi_log="Reset Transaksi Penjualan";
            $InputLog=addLog($Conn,$SessionIdAkses,$now,$kategori_log,$deskripsi_log);
            if($InputLog=="Success"){
                $response = [
                    "status" => "Success",
                    "message" => "Reset Transaksi Penjualan Berhasil!"
                ];
            }else{
                $response = [
                    "status" => "Error",

<?php
    ...
    if(empty($keyword_by)){
        if(empty($keyword)){
            $jml_data = mysqli_num_rows(mysqli_query($Conn, "SELECT*FROM transaksi_jual_beli"));
        }else{
            $jml_data = mysqli_num_rows(mysqli_query($Conn, "SELECT*FROM transaksi_jual_beli WHERE tanggal like '%$keyword%' OR kategori like '%$keyword%' OR metode like '%$keyword%' OR status like '%$keyword%'"));
        }
    }else{
        if(empty($keyword)){
            $jml_data = mysqli_num_rows(mysqli_query($Conn, "SELECT*FROM transaksi_jual_beli"));
        }else{
            $jml_data = mysqli_num_rows(mysqli_query($Conn, "SELECT*FROM transaksi_jual_beli WHERE $keyword_by like '%$keyword%'"));
        }
    }
?>
<script>
                            ...
                            if(empty($jml_data)){
                                ...
                                if(empty($keyword_by)){
                                    if(empty($keyword)){
                                        ...
                                        $query = mysqli_query($Conn, "SELECT*FROM transaksi_jual_beli WHERE tanggal like '%$keyword%' OR kategori like '%$keyword%' OR metode like '%$keyword%' OR status like '%$keyword%' ORDER BY $OrderBy $ShortBy LIMIT $posisi, $batas");
                                    }
                                }else{
                                    if(empty($keyword)){
                                        $query = mysqli_query($Conn, "SELECT*FROM transaksi_jual_beli ORDER BY $OrderBy $ShortBy LIMIT $posisi, $batas");
